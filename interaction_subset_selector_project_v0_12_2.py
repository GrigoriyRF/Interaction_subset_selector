...
class DecisionThresholdConfig:
    ...
    # report: threshold feasibility is diagnostic only; never removes a subset.
    # filter: prefer feasible subsets, but fall back to all confirmed subsets.
    # strict: require a feasible subset and fail if none exists.
    oos_policy: str = "report"
    # Backward-compatible legacy switch. When explicitly supplied without
    # oos_policy, AppConfig.from_dict translates True -> strict, False -> filter.
    require_oos_feasible: bool | None = None
    enforce_during_search: bool = False
...
class AppConfig:
    ...
    def from_dict(cls, raw: dict[str, Any]) -> "AppConfig":
        ...
        return cls(
            ...
            decision_threshold=DecisionThresholdConfig(
                **cls._normalize_decision_threshold_config(
    ...
    @staticmethod
    def _normalize_decision_threshold_config(raw: dict[str, Any]) -> dict[str, Any]:
        if not isinstance(raw, dict):
            raise TypeError("decision_threshold must be a YAML mapping")
        normalized = dict(raw)
        if "oos_policy" not in normalized and "require_oos_feasible" in normalized:
            normalized["oos_policy"] = (
                "strict" if bool(normalized["require_oos_feasible"]) else "filter"
            )
        return normalized
    ...
    def validate(self) -> None:
        ...
        if threshold.oos_policy not in {"report", "filter", "strict"}:
            raise ValueError(
                "decision_threshold.oos_policy must be report, filter, or strict"
            )
...
class CatBoostSubsetEvaluator:
    ...
    def _params(
        ...
        # CatBoost's eval_metric controls early stopping only. The selector
        # always computes the configured primary_metric itself from predict_proba
        # and uses that value for feature-subset ranking. Ranking metrics such as
        # PRAUC/AUC are not GPU-native in CatBoost and trigger CPU metric fallback.
        # Therefore every GPU fit uses Logloss only for early stopping; this does
        # not change the selector's actual subset-ranking metric.
        task_type = str(self.cfg.model_params.get("task_type", "CPU")).upper()
        if task_type == "GPU":
            eval_metric = "Logloss"
        else:
            eval_metric = {
    ...
    def _evaluate_many_raw(
        ...
        if len(evaluations) < minimum_successes:
            ...
            if failures:
                ...
                details = (
                    "; first failure: "
                    f"backend={first.backend}, subset_size={len(first.subset)}, "
                    f"attempts={first.attempts}, error_type={first.error_type}, "
                    f"error_message={first.error_message!r}, "
                    f"oom_detected={first.oom_detected}, "
                    f"termination_reason={first.termination_reason!r}, "
                    f"peak_worker_rss_gb={first.peak_worker_rss_gb:.3f}, "
                    f"peak_worker_gpu_gb={first.peak_worker_gpu_gb:.3f}"
                )
                _event(
                    "trial_failed",
                    fidelity=fidelity,
                    backend=first.backend,
                    subset_size=len(first.subset),
                    attempts=first.attempts,
                    error_type=first.error_type,
                    error_message=first.error_message,
                    oom_detected=first.oom_detected,
                    termination_reason=first.termination_reason,
                    peak_worker_rss_gb=first.peak_worker_rss_gb,
                    peak_worker_gpu_gb=first.peak_worker_gpu_gb,
                )
            raise RuntimeError(
                f"Only {len(evaluations)}/{len(subsets)} trials succeeded at "
                f"{fidelity}; at least {minimum_successes} are required{details}"
            )
        if failures:
            _event(
...
def run_preflight(config_path: str | Path) -> dict[str, Any]:
    ...
    # Baseline is bounded by the same feature budget as search. A full-universe
    # baseline can have a completely different RAM/VRAM profile and is outside
    # the configured search space when feature_count > search_feature_budget.
    baseline_feature_count = feature_budget
    baseline_dense_gb = (
    ...
    if feature_count > feature_budget:
        warnings.append(
            "The reference baseline is bounded to search_feature_budget; the full "
            "feature universe remains available to random/adaptive search"
        )
    if (
        cfg.execution.backend == "local"
    ...
    payload = {
        ...
        "workloads": workloads,
        "baseline": {
            "features": baseline_feature_count,
            "full_universe_features": feature_count,
            "bounded_to_search_feature_budget": bool(feature_count > feature_budget),
...
def run_pipeline(config_path: str | Path) -> None:
    ...
    # Use the already resource-calibrated, deterministic subset as the reference
    # baseline whenever the full feature universe exceeds search_max_features.
    # This keeps baseline and interaction discovery inside the measured resource
    # envelope while leaving the complete feature universe available to search.
    baseline_features = list(calibration_subset)
    _event(
        "baseline_started",
        n_features=len(baseline_features),
        full_universe_features=len(features),
        bounded=bool(len(baseline_features) < len(features)),
    )
    if resource_manager is not None:
    ...
    if isolate_local:
        ...
        baseline = baseline_runner._evaluate_many_raw(
            [frozenset(baseline_features)], "search"
        )[0]
    else:
        baseline = evaluator.evaluate(baseline_features, "search")
    _event("baseline_completed", valid_metric=baseline.valid_metric)
    interaction_cache = evaluator._interaction_cache_path(
        baseline_features, "search", cfg.search.interaction_pairs
    )
    ...
    if cfg.robust_validation.enabled and "oos" in samples:
        ...
        if cfg.decision_threshold.enabled:
            ...
            feasible_subsets = {
                ...
                and threshold_by_subset[subset].evaluation_feasible
            }
            threshold_gate_passed = bool(feasible_subsets)
            policy = cfg.decision_threshold.oos_policy
            used_unconstrained_fallback = False
            if policy == "report":
                eligible_subsets = set(confirmed_subsets)
            elif feasible_subsets:
                eligible_subsets = set(feasible_subsets)
            elif policy == "filter":
                eligible_subsets = set(confirmed_subsets)
                used_unconstrained_fallback = True
            else:  # strict
                eligible_subsets = set()
            ...
            _write_json(
                output / "threshold_validation.json",
                {
                    ...
                    "used_unconstrained_fallback": used_unconstrained_fallback,
                    "feasible_subsets": [
                        list(subset) for subset in sorted(feasible_subsets)
                    ],
                    "eligible_subsets": [
        ...
        selection_basis = (
            ...
            if cfg.decision_threshold.enabled
            and cfg.decision_threshold.oos_policy in {"filter", "strict"}
            and not used_unconstrained_fallback
