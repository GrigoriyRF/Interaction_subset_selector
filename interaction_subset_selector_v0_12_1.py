...
class CatBoostSubsetEvaluator:
    ...
    def _params(
        ...
        # and uses that value for feature-subset ranking. PRAUC is not implemented
        # on CatBoost GPU, so use a GPU-native surrogate for early stopping.
        task_type = str(self.cfg.model_params.get("task_type", "CPU")).upper()
        if task_type == "GPU":
            eval_metric = {
                "average_precision": "AUC:type=Classic;use_weights=true",
                "pr_auc": "AUC:type=Classic;use_weights=true",
                ...
                "brier": "Logloss:use_weights=true",
            }[self.primary_metric]
        else:
            eval_metric = {
